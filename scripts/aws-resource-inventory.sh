...
#
# Usage:
#   ./scripts/aws-resource-inventory.sh                    # Display current inventory
#   ./scripts/aws-resource-inventory.sh --export file.json # Export to JSON
#   ./scripts/aws-resource-inventory.sh --diff before.json # Compare against baseline
#   ./scripts/aws-resource-inventory.sh --verify-empty     # Verify zero resources (exit 1 if any remain)
#
# Prerequisites:
...
verify_empty() {
	...
	if [[ $total_resources -eq 0 ]]; then
		...
		echo ""
		log_error "Run './scripts/aws-nuclear-cleanup.sh' to force delete remaining resources"
		exit 1
	fi
}
