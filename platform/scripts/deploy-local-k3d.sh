...
#
# Usage:
#   ./platform/scripts/deploy-local-k3d.sh [--clean]
#
# Options:
...
TIMEOUT="120s"
LOCAL_PORT="8000"
MANIFEST_DIR="clusters/dev/bootstrap/k8s-manifests"
...
if [[ ! -f "${MANIFEST_DIR}/namespace.yaml" ]] || [[ ! -f "${MANIFEST_DIR}/deployment.yaml" ]] || [[ ! -f "${MANIFEST_DIR}/service.yaml" ]]; then
	error_exit "Kubernetes manifests not found in ${MANIFEST_DIR}/"
fi
...
echo "    kubectl delete -f ${MANIFEST_DIR}/deployment.yaml -f ${MANIFEST_DIR}/service.yaml"
echo ""
