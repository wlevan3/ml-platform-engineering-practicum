...
```bash
kubectl apply -f clusters/dev/bootstrap/k8s-manifests/manifests/
```
...
- `clusters/dev/bootstrap/k8s-manifests/deployment.yaml` - Deployment with resource requests/limits
- `clusters/dev/bootstrap/k8s-manifests/hpa.yaml` - Horizontal Pod Autoscaler (maxReplicas: 10)
- `infra/aws-core/terraform/modules/eks-cluster/` - EKS cluster setup
