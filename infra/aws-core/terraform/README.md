...
./infra/aws-core/terraform/scripts/bootstrap-backend.sh
...
2. **Update Kubernetes manifests** (see `clusters/dev/bootstrap/k8s-manifests/README.md`):

   - Update `clusters/dev/bootstrap/k8s-manifests/deployment.yaml` with ECR image URL
   - Change `imagePullPolicy: Never` → `IfNotPresent`
   - Create `clusters/dev/bootstrap/k8s-manifests/ingress.yaml` for ALB
