...
| <a name="input_cluster_endpoint_public_access"></a> [cluster\_endpoint\_public\_access](#input\_cluster\_endpoint\_public\_access) | Enable public API server endpoint. Required for CI/CD access (e.g., GitHub Actions). Production environments should set to false and use VPN/bastion. | `bool` | `false` | no |
| <a name="input_cluster_endpoint_public_access_cidrs"></a> [cluster\_endpoint\_public\_access\_cidrs](#input\_cluster\_endpoint\_public\_access\_cidrs) | List of CIDR blocks that can access the public API server endpoint. Must be explicitly configured if public access is enabled. | `list(string)` | `[]` | no |
...
| <a name="input_force_module_creation"></a> [force\_module\_creation](#input\_force\_module\_creation) | Force module creation even when the upstream terraform-aws-modules/eks gate would skip it. Leave true unless you explicitly need to disable provisioning (e.g., while stubbing this module). | `bool` | `true` | no |
